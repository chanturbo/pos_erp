...
class CouponCardPdfBuilder {
    ...
    if (paperSize == CouponPaperSize.a6) {
      doc.addPage(pw.Page(
        ...
        build: (ctx) => _buildCard(
          coupon: coupon, companyName: companyName, discountLabel: discountLabel,
          ttf: ttf, ttfRegular: ttfRegular, dateFmt: dateFmt, scale: 1.0,
        ),
      ));
    } else {
      ...
      doc.addPage(pw.Page(
        ...
        build: (ctx) => pw.Align(
          ...
          child: pw.SizedBox(
            ...
            child: _buildCard(
              coupon: coupon, companyName: companyName, discountLabel: discountLabel,
              ttf: ttf, ttfRegular: ttfRegular, dateFmt: dateFmt,
  ...
  static Future<pw.Document> buildMultiple(
    ...
    if (paperSize == CouponPaperSize.a6) {
      return _buildA6Multiple(coupons, companyName: companyName);
    } else {
      return _buildA4Multiple(coupons, companyName: companyName);
    }
  }
  ...
  static Future<pw.Document> _buildA6Multiple(
    ...
    for (final coupon in coupons) {
      doc.addPage(pw.Page(
        ...
        margin: pw.EdgeInsets.zero,
        build: (ctx) => _buildCard(
          coupon: coupon, companyName: companyName,
          ttf: ttf, ttfRegular: ttfRegular, dateFmt: dateFmt, scale: 1.0,
        ),
      ));
    }
            ...
            for (var c = 0; c < _cols; c++) {
              ...
              cells.add(pw.SizedBox(
                ...
                child: idx < batch.length
                    ? _buildCard(
                        coupon: batch[idx], companyName: companyName,
                        ttf: ttf, ttfRegular: ttfRegular, dateFmt: dateFmt,
                        scale: _scaleA4Grid,
                      )
                    : pw.Container(
                ...
                children: [
                  pw.Column(
                    ...
                    children: [
                      pw.Text(companyName,
                          style: pw.TextStyle(
                              font: ttfRegular, fontSize: companyFs, color: _kTextSub)),
                      pw.SizedBox(height: titleGap),
                      pw.Text('คูปองส่วนลด',
                          style: pw.TextStyle(font: ttf, fontSize: titleFs, color: _kNavy)),
                    ],
                  ),
                  if (discountLabel != null)
                    pw.Container(
                      ...
                      decoration: pw.BoxDecoration(
                          color: _kPrimary,
                          borderRadius: pw.BorderRadius.circular(6)),
                      child: pw.Text(discountLabel,
                          style: pw.TextStyle(
                              font: ttf, fontSize: titleFs, color: PdfColors.white)),
                    ),
                  if (coupon.promotionName != null)
                    pw.Text(coupon.promotionName!,
                        style: pw.TextStyle(
                            font: ttfRegular, fontSize: promoFs, color: _kTextSub)),
                  pw.Container(
                    ...
                    child: pw.Text(
                      coupon.expiresAt != null
                          ...
                          : 'ไม่มีวันหมดอายุ',
                      style: pw.TextStyle(
                          font: ttfRegular, fontSize: expiryFs, color: _kTextSub),
                    ),
                  ),
                ],
              ),
