...
class PurchaseOrderPdfBuilder {
      ...
      doc.addPage(pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            _buildTable(pages[pi],
                startNo: pi * rowsPerPage + 1, ttf: ttf, ttfR: ttfRegular),
            pw.Spacer(),
            _buildFooter(ttfR: ttfRegular),
          ],
        ),
      ));
    }
  ...
  static PdfColor _statusColor(String status) {
    switch (status) {
      case 'APPROVED':  return _kApproved;
      case 'PARTIAL':   return _kPartial;
      case 'COMPLETED': return _kDone;
      default:          return _kDraft;
    }
  }
  ...
  static pw.Widget _buildHeader({
    ...
    return pw.Column(
      ...
      children: [
        ...
        pw.Center(
          child: pw.Text('รายงานใบสั่งซื้อ',
  ...
  static pw.Widget _buildSummaryRow({
    ...
    pw.Widget cell(String label, String value, PdfColor vc) =>
        pw.Expanded(
          child: pw.Column(
    ...
    return pw.Table(
      ...
      children: [
        pw.TableRow(
          ...
          children: ['#', 'เลขที่ PO', 'ผู้จัดจำหน่าย',
                  'วันสั่งซื้อ', 'วันกำหนดส่ง', 'สถานะ', 'ยอดรวม (฿)']
              .map((h) => pw.Container(
                    padding: const pw.EdgeInsets.symmetric(
                        horizontal: 4, vertical: 6),
                    child: pw.Text(h,
                        style:
                            pw.TextStyle(font: ttf, fontSize: 8, color: _kText)),
                  ))
              .toList(),
        ),
        ...orders.asMap().entries.map((e) {
          ...
          return pw.TableRow(children: [
            ...
            cell(_date.format(o.poDate), ttfR,
                align: pw.Alignment.center, bgColor: bg),
            cell(o.deliveryDate != null ? _date.format(o.deliveryDate!) : '-',
                ttfR, align: pw.Alignment.center, bgColor: bg),
            cell(_statusLabel(o.status), ttf,
                align: pw.Alignment.center, color: sc, bgColor: bg),
            cell(_money.format(o.totalAmount), ttfR,
                align: pw.Alignment.centerRight, bgColor: bg),
          ]);
        }),
      ],
    );
  ...
  // ── Footer ───────────────────────────────────────────────────────
  static pw.Widget _buildFooter({required pw.Font ttfR}) =>
      pw.Container(
        padding: const pw.EdgeInsets.only(top: 6),
        decoration: const pw.BoxDecoration(
            border: pw.Border(
                top: pw.BorderSide(color: _kBorder, width: 0.5))),
        child: pw.Text('DEE POS — รายงานใบสั่งซื้อ',
