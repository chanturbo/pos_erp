...
class PurchaseReturnPdfBuilder {
  ...
  static Future<pw.Document> build(
    ...
    final doc      = pw.Document(title: 'รายงานการคืนสินค้า', author: companyName);
    ...
    final summaryRow = _buildSummaryRow(
      count: returns.length, confirmed: confirmed,
      total: totalAmt, ttf: ttf, ttfR: ttfR,
    );

    const rowsPerPage = 34;
    final pages = <List<PurchaseReturnModel>>[];
    for (var i = 0; i < returns.length; i += rowsPerPage) {
      pages.add(returns.sublist(
          i, (i + rowsPerPage) > returns.length ? returns.length : i + rowsPerPage));
    }
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            _buildHeader(companyName: companyName, printedAt: printedAt,
                page: pi + 1, totalPages: totalPages,
                summaryLine: summaryLine, summaryRow: summaryRow,
                ttf: ttf, ttfR: ttfR),
            _buildTable(pages[pi], startNo: pi * rowsPerPage + 1, ttf: ttf, ttfR: ttfR),
            pw.Spacer(),
            _buildFooter(ttfR: ttfR),
          ],
        ),
      ));
    }
  ...
  static pw.Widget _buildHeader({
    required String companyName, required String printedAt,
    required int page, required int totalPages,
    required String summaryLine, required pw.Widget summaryRow,
    required pw.Font ttf, required pw.Font ttfR,
    ...
    pw.Center(child: pw.Text(companyName,
        style: pw.TextStyle(font: ttfR, fontSize: 9, color: _kSub))),
    pw.SizedBox(height: 2),
    pw.Center(child: pw.Text('รายงานการคืนสินค้า',
        style: pw.TextStyle(font: ttf, fontSize: 14, color: _kText))),
    pw.SizedBox(height: 2),
    pw.Center(child: pw.Text(summaryLine,
        style: pw.TextStyle(font: ttfR, fontSize: 8, color: _kSub))),
  ...
  static pw.Widget _buildSummaryRow({
    required int count, required int confirmed,
    required double total, required pw.Font ttf, required pw.Font ttfR,
  }) {
    pw.Widget cell(String label, String value, PdfColor vc) => pw.Expanded(
      child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
        pw.Text(label, style: pw.TextStyle(font: ttfR, fontSize: 8, color: _kSub)),
        pw.SizedBox(height: 2),
        pw.Text(value, style: pw.TextStyle(font: ttf, fontSize: 10, color: vc)),
      ]),
    );
    return pw.Container(
  ...
  static pw.Widget _buildTable(
    List<PurchaseReturnModel> returns, {
    required int startNo, required pw.Font ttf, required pw.Font ttfR,
  }) {
    const colWidths = {
    ...
    pw.Widget cell(String text, pw.Font font, {
      ...
      PdfColor? color, PdfColor? bgColor,
    }) => pw.Container(
      color: bgColor,
    ...
    return pw.Table(
      ...
      children: [
        pw.TableRow(
          ...
          children: ['#', 'เลขที่คืน', 'ซัพพลายเออร์', 'วันที่คืน', 'สถานะ', 'ยอดรวม (฿)']
              .map((h) => pw.Container(
        ...
        ...returns.asMap().entries.map((e) {
          ...
          return pw.TableRow(children: [
            cell('${startNo + i}', ttfR, align: pw.Alignment.center, bgColor: bg),
            cell(r.returnNo, ttf, bgColor: bg),
            cell(r.supplierName, ttfR, bgColor: bg),
            cell(_date.format(r.returnDate), ttfR,
                align: pw.Alignment.center, bgColor: bg),
            cell(r.isConfirmed ? 'ยืนยันแล้ว' : 'ร่าง', ttf,
                align: pw.Alignment.center, color: sc, bgColor: bg),
            cell(_money.format(r.totalAmount), ttfR,
                align: pw.Alignment.centerRight, color: _kRed, bgColor: bg),
          ]);
        }),
      ],
    );
  ...
  static pw.Widget _buildFooter({required pw.Font ttfR}) => pw.Container(
    padding: const pw.EdgeInsets.only(top: 6),
    decoration: const pw.BoxDecoration(
        border: pw.Border(top: pw.BorderSide(color: _kBorder, width: 0.5))),
    child: pw.Text('DEE POS — รายงานการคืนสินค้า',
