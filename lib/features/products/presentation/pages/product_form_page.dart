...
class _ProductFormPageState extends ConsumerState<ProductFormPage> {
  ...
  PrepStation? _prepStation;
  bool _requiresPreparation = false;
  bool _dineInAvailable = true;
  bool _takeawayAvailable = true;
  bool _isLoading = false;
  bool _codeAutoGenerated = false;

  String? _imagePath;
  Set<String> _linkedGroupIds = {}; // modifier groups ที่ผูกกับสินค้านี้
  ...
  void initState() {
    ...
    _price1Controller = TextEditingController(
      text: p?.priceLevel1.toString() ?? '0',
    );
    _price2Controller = TextEditingController(
      text: p?.priceLevel2.toString() ?? '0',
    );
    _price3Controller = TextEditingController(
      text: p?.priceLevel3.toString() ?? '0',
    );
    _price4Controller = TextEditingController(
      text: p?.priceLevel4.toString() ?? '0',
    );
    _price5Controller = TextEditingController(
      text: p?.priceLevel5.toString() ?? '0',
    );
    _costController = TextEditingController(
      text: p?.standardCost.toString() ?? '0',
    );
    ...
    if (p != null) {
      ...
      _serviceMode = ServiceMode.fromString(p.serviceMode);
      _prepStation = PrepStation.fromString(p.prepStation);
      _requiresPreparation = p.requiresPreparation;
      _dineInAvailable = p.dineInAvailable;
      _takeawayAvailable = p.takeawayAvailable;
      // โหลด linked modifier groups จะทำใน build() ผ่าน provider
    }
  ...
  void _autoGenerateCode() {
    final barcode = _barcodeController.text.trim();
    setState(() {
      _codeController.text = barcode.isNotEmpty ? barcode : _generateCode();
      _codeAutoGenerated = true;
    });
  }
  ...
  Future<void> _handleSubmit() async {
    ...
    final data = {
      ...
      'service_mode': _serviceMode.value,
      'prep_station': _prepStation?.value,
      'requires_preparation': _requiresPreparation,
      'dine_in_available': _dineInAvailable,
      'takeaway_available': _takeawayAvailable,
    };

    final bool success;
    String? savedProductId = widget.product?.productId;
    if (widget.product != null) {
      success = await ref
          .read(productListProvider.notifier)
          .updateProduct(widget.product!.productId, data);
    } else {
      // create — ต้องได้ productId จาก response
      final notifier = ref.read(productListProvider.notifier);
      success = await notifier.addProduct(data);
      if (success) {
        // หา productId ที่เพิ่งสร้าง จาก product list ล่าสุด
        final products = ref.read(productListProvider).value ?? [];
        final code = (data['product_code'] as String?) ?? '';
        savedProductId = products
    ...
    if (success && savedProductId != null) {
      final modifierNotifier = ref.read(modifierGroupsProvider.notifier);
      final allGroups = ref.read(modifierGroupsProvider).value ?? [];
      for (final g in allGroups) {
        final gid = g.modifierGroupId;
        if (_linkedGroupIds.contains(gid)) {
          await modifierNotifier.linkGroup(savedProductId, gid);
        } else {
          await modifierNotifier.unlinkGroup(savedProductId, gid);
        }
      }
    }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      LayoutBuilder(
                        builder: (ctx, bc) {
                          final isWide = bc.maxWidth >= 1220;
                          final isMedium = bc.maxWidth >= 900;
                          ...
                          final c3 = Column(
                            children: [
                              ...
                              _SectionCard(
                                ...
                                iconColor: const Color(0xFFE57200),
                                ...
                                child: Column(
                                  children: [
                                          ...
                                          : (value) {
                                              ...
                                              setState(() {
                                                ...
                                                if (value ==
                                                    ServiceMode.retail) {
                                                  _prepStation = null;
                                                  _requiresPreparation = false;
                                                  _dineInAvailable = false;
                                                  _takeawayAvailable = false;
                                                } else {
                                    ...
                                    DropdownButtonFormField<PrepStation?>(
                                      initialValue: _prepStation,
                                      decoration: const InputDecoration(
                                        labelText: 'Station เตรียมอาหาร',
                                        border: OutlineInputBorder(),
                                        prefixIcon: Icon(Icons.kitchen_outlined),
                                      ),
                                      items: [
                                        const DropdownMenuItem<PrepStation?>(
                                          value: null,
                                          child: Text('ไม่ระบุ'),
                                        ),
                                        ...PrepStation.values.map(
                                          (station) =>
                                              DropdownMenuItem<PrepStation?>(
                                            value: station,
                                      ...
                                      onChanged: _isLoading ||
                                              _serviceMode == ServiceMode.retail
                                          ? null
                                          : (value) => setState(
                                                () => _prepStation = value,
                                              ),
                                    ...
                                    const SizedBox(height: 12),
                                    _SwitchRow(
                                      label: 'ต้องผ่านการเตรียม',
                                      subLabel: 'ใช้กับเมนูที่ต้องเข้าครัวหรือบาร์',
                                      value: _requiresPreparation,
                                      activeColor: const Color(0xFFE57200),
                                      onChanged: _serviceMode ==
                                                  ServiceMode.retail ||
                                              _isLoading
                                          ? null
                                          : (v) => setState(
                                                () => _requiresPreparation = v,
                                              ),
                                    ),
                                    const Divider(height: 16),
                                    _SwitchRow(
                                      label: 'ขายแบบทานที่ร้าน',
                                      subLabel: 'ใช้ใน dine-in order',
                                    ...
                                    const Divider(height: 16),
                                    _SwitchRow(
                                      label: 'ขายแบบซื้อกลับบ้าน',
                                      subLabel: 'ใช้ใน takeaway order',
                                      value: _takeawayAvailable,
                          ...
                          final asyncLinked = productId != null
                              ? ref.watch(productLinkedGroupIdsProvider(productId))
                              : null;
                          // sync linked ids เข้า state ครั้งแรก
                          asyncLinked?.whenData((ids) {
                            if (_linkedGroupIds.isEmpty && ids.isNotEmpty) {
                              WidgetsBinding.instance.addPostFrameCallback(
                                  (_) => setState(() => _linkedGroupIds = Set.from(ids)));
                            }
                          });
                          final asyncAllGroups = ref.watch(modifierGroupsProvider);
                          final allGroups = asyncAllGroups.value ?? [];

                          final cModifier = allGroups.isEmpty
                              ...
                              : _SectionCard(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        'เลือก group ที่ลูกค้าต้องเลือกตอนสั่ง',
                                        style: TextStyle(
                                            fontSize: 12,
                                      ...
                                      ...allGroups.map((g) => CheckboxListTile(
                                            ...
                                            subtitle: Text(
                                              ...
                                              '${g.isSingle ? "เลือกได้ 1" : "เลือกได้หลาย"}'
                                              '${g.isRequired ? " · บังคับ" : ""}',
                                              style: TextStyle(
                                                  fontSize: 11,
                                                  color: AppTheme.subtextColorOf(context)),
                                            ),
                                            value: _linkedGroupIds.contains(g.modifierGroupId),
                                            onChanged: _isLoading
                                                ? null
                                                : (v) => setState(() {
                                                      if (v == true) {
                                                        _linkedGroupIds.add(g.modifierGroupId);
                                                      } else {
                                                        _linkedGroupIds.remove(g.modifierGroupId);
                                                      }
                                                    }),
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: const Text('สแกนบาร์โค้ด', style: TextStyle(fontSize: 13)),
            style: ElevatedButton.styleFrom(
        ...
        // ── Text field บาร์โค้ด ──────────────────────────────
        FocusTraversalOrder(
          order: NumericFocusOrder(focusOrder ?? 1),
          child: TextFormField(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        // TextField
        FocusTraversalOrder(
          order: NumericFocusOrder(focusOrder ?? 2),
          child: TextFormField(
            controller: controller,
            enabled: enabled,
...
class _PriceRow extends StatelessWidget {
  ...
  final String h2;
  final bool enabled;
  final double? f1Order;
  final double? f2Order;
...
class _ImagePickerWidget extends StatelessWidget {
  ...
  Widget _placeholder() => const Column(
    ...
    children: [
      Icon(Icons.add_photo_alternate_outlined, size: 32, color: Colors.white),
      SizedBox(height: 6),
      Text(
        'กดเพื่อเลือกรูป',
