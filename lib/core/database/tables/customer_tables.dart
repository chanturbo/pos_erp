...
class Customers extends Table {
  ...
  TextColumn get customerGroupId =>
      text().nullable().references(CustomerGroups, #customerGroupId)();
...
class Suppliers extends Table {
  ...
  TextColumn get supplierCode => text().named('supplier_code')();
  ...
  TextColumn get lineId =>
      text().named('line_id').nullable()(); // ✅ เพิ่มบรรทัดนี้
  TextColumn get address => text().nullable()();
  TextColumn get taxId => text().named('tax_id').nullable()();
  IntColumn get creditTerm =>
      integer().named('credit_term').withDefault(const Constant(30))();
  RealColumn get creditLimit =>
      real().named('credit_limit').withDefault(const Constant(0))();
  RealColumn get currentBalance =>
      real().named('current_balance').withDefault(const Constant(0))();
