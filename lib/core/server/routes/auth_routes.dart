...
class AuthRoutes {
  ...
  Future<Response> _loginHandler(Request request) async {
    try {
      ...
      final userCountRow = await db
          .customSelect('SELECT COUNT(*) AS total FROM users')
          .getSingle();
      final userCount = userCountRow.data['total'] as int? ?? 0;
      if (userCount == 0) {
        return Response(
          409,
          body: jsonEncode({
            'success': false,
            'message':
                'ยังไม่มีผู้ใช้ในระบบ ขณะนี้เครื่องนี้น่าจะถูกล้างข้อมูลพร้อมเปิด Skip seed อยู่ กรุณา restore backup หรือปิด Skip seed แล้ว Factory Reset ใหม่',
          }),
  ...
  Future<Response> _logoutHandler(Request request) async {
    try {
      ...
      if (token == null) {
        return Response(
          400,
          body: jsonEncode({'success': false, 'message': 'ไม่พบ token'}),
        );
      }
