...
class KitchenRoutes {
  ...
  Future<Response> _getQueue(Request req) async {
    try {
      ...
      final itemsQuery = db.select(db.salesOrderItems)
        ..where((i) =>
            i.orderId.isIn(orderIds) & i.kitchenStatus.isIn(statuses));
      ...
      final products = await (db.select(db.products)
            ..where((p) => p.productId.isIn(productIds)))
          .get();
      ...
      final tables = tableIds.isNotEmpty
          ? await (db.select(db.diningTables)
                ..where((t) => t.tableId.isIn(tableIds)))
              .get()
          : <DiningTable>[];
      final tableMap = {
        for (final t in tables)
          t.tableId: t.tableDisplayName ?? t.tableNo,
      };
      ...
      final result = items.where((i) {
        ...
        final needsPrep =
            (product?.requiresPreparation ?? false) || product?.prepStation != null;
        ...
        return {
          ...
          'table_name': order?.tableId != null ? tableMap[order!.tableId] : null,
      ...
      result.sort((a, b) => (a['created_at'] as String)
  ...
  Future<Response> _getSummary(Request req) async {
    try {
      ...
      final filteredOrderIds = (branchId != null
  ...
  Future<Response> _updateItemStatus(Request req, String itemId) async {
    try {
      ...
      final validStatuses = ['PENDING', 'PREPARING', 'READY', 'SERVED', 'CANCELLED'];
      ...
      final item = await (db.select(db.salesOrderItems)
      ...
      return _ok({'item_id': itemId, 'status': newStatus});
    } catch (e) {
      return _err(e);
    }
  }
  ...
  Future<Response> _getAnalytics(Request req) async {
    try {
      ...
      final targetDate = dateStr != null ? DateTime.tryParse(dateStr) : DateTime.now();
      final dayStart = DateTime(targetDate!.year, targetDate.month, targetDate.day);
      final dayEnd = dayStart.add(const Duration(days: 1));

      final orders = await (db.select(db.salesOrders)
            ..where((o) {
              Expression<bool> cond = o.orderDate.isBiggerOrEqualValue(dayStart) &
      ...
      final preparedItems = allItems.where((i) => i.preparedAt != null).toList();
      double totalPrepMins = 0;
      for (final item in preparedItems) {
        final order = restaurantOrders.firstWhere((o) => o.orderId == item.orderId);
        final prepTime = item.preparedAt!.difference(order.createdAt).inSeconds / 60.0;
        if (prepTime > 0) totalPrepMins += prepTime;
      }
      final avgPrepMins = preparedItems.isNotEmpty ? totalPrepMins / preparedItems.length : 0.0;
      ...
      for (final order in restaurantOrders) {
        final items = allItems.where((i) => i.orderId == order.orderId).toList();
        final servedItems = items.where((i) => i.kitchenStatus == 'SERVED' && i.preparedAt != null);
        if (servedItems.isNotEmpty) {
          final lastServed = servedItems.map((i) => i.preparedAt!).reduce((a, b) => a.isAfter(b) ? a : b);
          final orderTime = lastServed.difference(order.createdAt).inSeconds / 60.0;
          if (orderTime > 0) {
      ...
      final avgOrderMins = ordersWithTime > 0 ? totalOrderMins / ordersWithTime : 0.0;
      ...
      for (final item in allItems) {
        ...
        if (item.preparedAt != null) {
          final order = restaurantOrders.firstWhere((o) => o.orderId == item.orderId);
          final mins = item.preparedAt!.difference(order.createdAt).inSeconds / 60.0;
          if (mins > 0) {
      ...
      final avgPrepByStation = prepTimeByStation.map((station, times) =>
      ...
      return _ok({
        ...
        'avg_prep_by_station': avgPrepByStation.map((k, v) =>
            MapEntry(k, double.parse(v.toStringAsFixed(1)))),
        'top_items': topItems,
      });
    } catch (e) {
      return _err(e);
